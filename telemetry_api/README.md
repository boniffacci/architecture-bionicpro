...
├── test_telemetry_api.py    # Автотесты
└── README.md                # Этот файл
```
...
2. ✅ `test_add_single_telemetry_event` — добавление одного события
3. ✅ `test_add_multiple_telemetry_events` — добавление нескольких событий
4. ✅ `test_add_empty_events_list` — валидация пустого списка
5. ✅ `test_add_events_with_different_prosthesis_types` — разные типы протезов
6. ✅ `test_saved_ts_is_set_automatically` — автоматическая установка saved_ts
7. ✅ `test_missing_required_fields` — валидация обязательных полей
8. ✅ `test_large_batch_of_events` — пакетная загрузка 100 событий
