...
### Тест локального пользователя

```bash
# 1. Перезапустить Keycloak
docker compose restart keycloak

# 2. Открыть фронтенд
open http://localhost:3000

# 3. Попытаться войти как prosthetic1:prosthetic123
# 4. Должен появиться экран настройки OTP
# 5. Использовать Google Authenticator для сканирования QR
# 6. Ввести код из приложения
# 7. Успешный вход
```

### Тест LDAP-пользователя

```bash
# 1. Попытаться войти как customer2:customer2_password
# 2. Должен появиться экран настройки OTP
# 3. Настроить OTP
# 4. Успешный вход
```
