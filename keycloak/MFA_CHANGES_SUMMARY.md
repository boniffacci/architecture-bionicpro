...
### ⚠️ Важно: Интеграционные тесты потребуют обновления

Тесты, которые выполняют вход пользователей, теперь будут требовать:
1. Настройку OTP при первом входе
2. Ввод OTP-кода при каждом последующем входе

### Рекомендации для тестов:
1. **Отключить MFA для тестовых пользователей** (в тестовом окружении)
2. **Использовать фиксированный TOTP secret** для предсказуемых кодов
3. **Мокировать OTP-проверку** в тестах

### Пример отключения MFA для тестов:
```python
# В conftest.py
@pytest.fixture(scope="session", autouse=True)
def disable_mfa_for_tests():
    """Отключает MFA для тестовых пользователей."""
    # Удалить requiredActions через Keycloak Admin API
    # или использовать отдельный realm-export для тестов
    pass
```
