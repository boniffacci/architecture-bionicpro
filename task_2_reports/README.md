...
Здесь нам предложили вполне конкретный набор технологий и решений:

- Clickhouse в качестве OLAP-базы;  
- ETL-процесс на airflow для переноса данных из CRM DB и Telemetry DB в OLAP-базу;  
- отдельный микросервис Reports API для построения отчётов.

Укажем это на нашей доработанной drawio-диаграмме

![container_diagram_with_olap.drawio.svg](container_diagram_with_olap.drawio.svg)
_Диаграмма контейнеров с добавлением Reports API, OLAP-базы и ETL-процесса (жёлтым выделены новые системы нашей разработки, серым -- системы сторонних производителей, которые мы только разворачиваем)_

Описание нашего прототипа будет ниже.

### Прототип CRM API

В настоящем бэкэнде CRM генерировались бы данные:
...
Нам для нашей задачи хватит одной таблицы users с информацией о пользователях.

У нас будет сервис-болванка crm_api, написанный на FastAPI и расположенный в директории `crm_api`. У него есть 2 эндпоинта:

* `/populate_base` (заполнить свою тестовую Postgres-базу из файла crm.csv), первые несколько юзеров будут юзеры `prosthetic1`, `prosthetic2`, `customer1`, `customer2`, `customer3` (user_uuid взяты либо те же, что стоят в realm-export.json, либо те же, что стоят в настройках LDAP для этих юзеров)  
* `/register`: зарегистрировать одного пользователя с заданными настройками (используется только для автотестов).

Проверку JWT я здесь не делал, её будем делать только в Reports API.

### Прототип Telemetry API

В настоящем бэкэнде телеметрии генерировались бы разнообразные данные по телеметрии, но нам для нашей задачи тоже хватит одной таблицы telemetry_events с “данными о сигналах”, полученных от протезов.

Для этого у нас будет ещё один сервис-болванка telemetry_api, писанный на FastAPI и расположенный в директории `telemetry_api`. У него тоже 2 эндпоинта:
...
Проверку JWT я и здесь не делал, её будем делать только в Reports API.

### Прототип OLAP-базы на ClickHouse

В боевой системе оно бы развёртывалось в Kubernetes из Helm Chart, и там было бы хитрое шардирование, репликация и все 33 удовольствия – но в нашем случае мы просто развернём одну ноду в Docker Compose (см. секцию `olap-db`).
...
- `telemetry_events` (`ReplacingMergeTree`):  
  ...
  - выбираем именно `ReplacingMergeTree`, чтобы обеспечить уникальность событий на случай повторного чтения из kafka (потом пригодится).
...
Для нашего прототипа мы ~~не будем купать лошадей в шампанском, а обольём кота пивом~~ просто развернём в нашем docker compose отладочную конфигурацию airflow standalonе, причём с отключенной аутентификацией.
...
![airflow_7.png](airflow_7.png)
_Потом возвращаемся в [http://localhost:3000](http://localhost:3000) и жмём кнопку “Отчёт (default)”, должны показать вот такой отчёт из reports_api (по запросу из `olap-db`, из схемы `default`)_
...
Приложение `reports_api` запускается через `docker compose` (см. секцию `reports-api` в `docker-compose.yaml`), исходники находятся в [`reports_api/`](../reports_api)

Приложение написано на FastAPI, у него 2 основных эндпоинта:

- `/jwt`: вывести содержимое JWT-токена, который выставлен на auth_proxy (нужно для отладки и тестирования);  
...
![restriction_1.png](restriction_1.png)
...
![restriction_2.png](restriction_2.png)
...
![restriction_3.png](restriction_3.png)
...
Кнопки генерации отчётов реализованы в `bionicpro_frontend/src/App.tsx`:
...
3. Проверяет наличие отчёта в MinIO-кэше через `nginx_minio_proxy`
4. Если отчёт в кэше — загружает его (помечает `from_cache: true`)
5. Если отчёта нет — генерирует новый через `reports_api/reports` (помечает `from_cache: false`)
