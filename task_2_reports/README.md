...
Проверку JWT я здесь не делал, её будем делать только в `reports_api`.
...
Для этого у нас будет ещё один сервис-болванка [`telemetry_api`](../telemetry_api), написанный на FastAPI и расположенный в директории [`telemetry_api`](../telemetry_api). У него тоже 2 эндпоинта:
...
Проверку JWT я и здесь не делал, её будем делать только в `reports_api`.
...
Для нашего прототипа мы ~~не будем купать лошадей в шампанском, а обольём кота пивом~~ просто развернём в нашем [`docker-compose`](../docker-compose.yaml) отладочную конфигурацию `airflow` standalone, причём с отключенной аутентификацией.
